...
export default function AdminFunnelPage() {
  ...
  function abrirNuevaSubetapa(etapaId: string) {
    ...
    requestAnimationFrame(() => document.getElementById('administracion-etapa')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3">
            {etapasActivas.filter((etapa) => !etapaSeleccionada || etapa.id === etapaSeleccionada).map((etapa) => {
              // La ficha resume todos los subestados de la etapa, incluidos los
              // inactivos, para que la relación histórica nunca quede oculta.
              const todosLosHijos = subestados
                .filter((s) => s.etapa_id === etapa.id)
                .sort((a, b) => a.orden - b.orden);
              const hijos = todosLosHijos.filter((s) => s.activo);
