...
export default async function PipelineYFunnelPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: etapas }, { data: oportunidades }, { data: subestados }] = await Promise.all([
    ...
    supabase
      .from('oportunidades')
      .select('id, etapa_id, subestado_id, estado, actualizado_en')
      .eq('estado', 'activa'),
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {etapasActivas.map((etapa, index) => {
              ...
              const lentas = items.filter((o) => hoursSince(o.actualizado_en) >= 24 && hoursSince(o.actualizado_en) < 48).length;
              const estancadas = items.filter((o) => hoursSince(o.actualizado_en) >= 48).length;
      ...
      {vista === 'configuracion' && (
        <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {sesion.esSuper && <Link href="/admin/funnel" className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white">Administrar configuración</Link>}
          </div>
